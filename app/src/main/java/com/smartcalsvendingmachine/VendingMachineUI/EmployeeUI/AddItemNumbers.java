...
public class AddItemNumbers extends Activity {

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_add_item_numbers);
        TableLayout table = (TableLayout) findViewById(R.id.item_table);
        TextView id, name, price;
        EditText capacity, quantity;
        Item item;

        for (int i = 0; i < AddItems.items.size(); i++) {
            item = AddItems.items.get(i);
            if (!item.isChosen()) {
                continue;
            }
    ...
    private class AddTask extends AsyncTask<Void, Void, String> {
        ...
        protected void onPostExecute(String result) {
            if (result != null) {
                ...
                Intent intent = new Intent(AddItemNumbers.this, EmployeeOptions.class);
                intent.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
                startActivity(intent);
            } else {
                toast("Error.");
