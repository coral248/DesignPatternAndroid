...
public class CustomerSearchResult extends Activity implements LoaderManager.LoaderCallbacks<Cursor> {

    private static final int SORT_BY_CAL = 0;
    private static final int SORT_BY_SUGAR = 1;

    private int cmin, cmax, smin, smax;
    private int sort;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        cmin = intent.getIntExtra("cmin", 0);
        cmax = intent.getIntExtra("cmax", 1000);
        smin = intent.getIntExtra("smin", 0);
        smax = intent.getIntExtra("smax", 100);
            ...
            public void onCheckedChanged(RadioGroup group, int checkedId) {
                switch (checkedId){
                    case R.id.sort_by_calories:
                        sort = SORT_BY_CAL;
                        getLoaderManager().restartLoader(0, null, CustomerSearchResult.this);
                        break;
    ...
    public Loader<Cursor> onCreateLoader( int id, Bundle args){
        if(sort == SORT_BY_CAL){
            return new CursorLoader(this, VendingMachineContentProvider.CONTENT_URI,
                    null, VendingMachineDatabase.ITEM_CAL + ">=? AND "
                    + VendingMachineDatabase.ITEM_CAL + "<=? AND "
                    + VendingMachineDatabase.ITEM_SUGAR + ">=? AND "
                    + VendingMachineDatabase.ITEM_SUGAR + "<=? AND "
                    + VendingMachineDatabase.ITEM_QUANTITY + ">0",
                    new String[] {String.valueOf(cmin), String.valueOf(cmax), String.valueOf(smin), String.valueOf(smax)},
                    VendingMachineDatabase.ITEM_CAL + " ASC");
            ...
            return new CursorLoader(this, VendingMachineContentProvider.CONTENT_URI,
                    ...
                    + VendingMachineDatabase.ITEM_QUANTITY + ">0",
                    new String[] {String.valueOf(cmin), String.valueOf(cmax), String.valueOf(smin), String.valueOf(smax)},
                    VendingMachineDatabase.ITEM_SUGAR + " ASC");
        }
        return null;
    }
